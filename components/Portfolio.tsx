      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 lg:gap-12">
          {PORTFOLIO_ITEMS.map((item) => {
            const imageSrc = item.id === 1 ? medicareImg : item.imageUrl;
            ...
            return (
              <ItemWrapper
                key={item.id}
                {...wrapperProps}
                className="group relative rounded-2xl overflow-hidden cursor-pointer bg-zinc-900 border border-white/5 block"
              >
